(() => {
  ...
  const LEGACY_MIGRATION_KEY =
    "teacherDashboard3.participationLegacyMigration.v1";
  ...
  function createDefaultParticipation() {
    ...
    PARTICIPATION_TERMS.forEach(definition => {
      terms[definition.id] =
        createDefaultParticipationTerm(definition);
    });
    ...
    PARTICIPATION_TERMS.forEach(
      definition => {
        ...
        const normalizedTerm =
          createDefaultParticipationTerm(
            definition
          );
  ...
  function hasAnyParticipationScores(data) {
    return PARTICIPATION_TERMS.some(
      definition =>
        Object.values(
          data.participation
            .terms[
  ...
  function migrateLegacyTrackerData(data) {
    ...
    try {
      ...
      console.warn(
        "Legacy Participation Tracker data could not be read.",
        error
      );
    ...
    const officialHadScores =
      hasAnyParticipationScores(
        data
      );
  ...
  function load() {
    try {
      ...
      /*
        Dashboard 3.0 has its own shared-data key so development
        work does not modify Dashboard 2.0.

        On the first 3.0 load only, copy the existing Dashboard
        2.0 shared data as the starting point.
      */
  ...
  function getParticipationTerms() {
    ...
    return PARTICIPATION_TERMS.map(
      definition => ({
        id:
          definition.id,
  ...
  function setActiveParticipationTerm(
    termId
  ) {
  ...
  function resetStudentPickerSchoolYearState(
    rawText
  ) {
  ...
  function resetParticipationUIState(
    rawText
  ) {
  ...
  function getStorageValuesByPrefix(
    prefix
  ) {
  ...
  function removeStorageByPrefix(
    prefix
  ) {
  ...
  function restoreStorageValuesByPrefix(
    prefix,
    values
  ) {
  ...
  function createSchoolYearBackup(
    sharedData
  ) {
  ...
  function newSchoolYearReset() {
    ...
    /*
      School-wide bell timing is configuration,
      so it survives the reset.
    */

    next.bellSchedules =
      clone(
        before.bellSchedules
      );


    /*
      Scoreboard championship/results data
      belongs to the old school year.
    */
    ...
    /*
      Clear old rosters, picker pools/history/groups while
      preserving picker style/target choices and each
      period's preferred group configuration.
    */
    ...
    /*
      Participation scores are cleared in shared data.

      The Tracker's undo history is also cleared so last
      year's students cannot be restored.
    */
    ...
    /*
      Agenda entries belong to the old school year.

      The backup contains all Dashboard 3.0 Agenda date
      records so Undo New School Year can restore them.
    */
    ...
    /*
      Armed warnings/current bell target are session state,
      not preferences.
    */
    ...
    /*
      Prevent the very old Participation Tracker format
      from being migrated back into the freshly reset
      Dashboard.
    */
  ...
  function undoNewSchoolYearReset() {
    ...
    /*
      Version 3 backs up the complete Dashboard 3.0
      school-year state, including all date-based
      Agenda records.
    */
    ...
    /*
      Backward compatibility for the earlier
      version-2 school-year backup.
    */
    ...
    /*
      Backward compatibility for the first
      reset-backup format, which stored only
      DashboardData.
    */
